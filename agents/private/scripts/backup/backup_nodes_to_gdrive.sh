...
gdrive_upload() {
    ...
    echo "UPLOAD_RESPONSE: $response" >&2
    python3 -c "import sys,json; r=json.load(sys.stdin); print('OK' if 'id' in r else 'FAIL: '+str(r))" <<< "$response" 2>/dev/null
}
...
main() {
    ...
    # Build archive with agents/ wrapper for horc-restore compatibility.
    # Structure:
    #   agents/registry.json
    #   agents/envs/<node>.env
    #   agents/nodes/<node>/.hermes/    (excluding per-node skills/)
    #   agents/private/                  (shared local orchestrator state)
    # Excludes hermes-agent/ runtime copies, workspace/, logs/, node metadata —
    # these are reinstalled/rebuilt on restore, not restored from backup.
    #
    # Multi-node: we stage into a temp dir so all nodes go into one archive.
    STAGE_DIR="$(mktemp -d)"
    trap 'rm -rf "$STAGE_DIR"' EXIT

    # Copy registry + envs once
    mkdir -p "$STAGE_DIR/agents/envs" "$STAGE_DIR/agents/nodes" "$STAGE_DIR/agents/private"
    [[ -f /local/agents/registry.json ]] && cp /local/agents/registry.json "$STAGE_DIR/agents/"
    cp /local/agents/envs/*.env "$STAGE_DIR/agents/envs/" 2>/dev/null || true
    if [[ -d /local/agents/private ]]; then
        rsync -a \
            --exclude='__pycache__' \
            --exclude='*.pyc' \
            --exclude='.pytest_cache' \
            "/local/agents/private/" \
            "$STAGE_DIR/agents/private/" \
            2>&1 | tee -a "$LOGFILE"
    fi

    for node in "${BACKUP_NODES[@]}"; do
        mkdir -p "$STAGE_DIR/agents/nodes/${node}"
        # rsync with excludes: preserves structure, fast, clean
        rsync -a \
            --exclude='*.tar.gz' \
            --exclude='__pycache__' \
            --exclude='.pyc' \
            ...
            --exclude='*.db-wal' \
            --exclude='*.db-shm' \
            "/local/agents/nodes/${node}/.hermes/" \
            "$STAGE_DIR/agents/nodes/${node}/" \
            2>&1 | tee -a "$LOGFILE"
    done

    tar -czf "$ARCHIVE_PATH" -C "$STAGE_DIR" agents/ 2>&1 | tee -a "$LOGFILE"
    ...
    # Step 5: Delete prior daily backups in Drive BEFORE uploading new one.
    # This avoids the bug where the new upload gets caught by the delete query
    # (since the query runs against ALL files currently in the folder).
