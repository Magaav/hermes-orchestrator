...
PLUGIN_ROOT = Path("/local/plugins/hermes-core")
...
CLONE_MANAGER_PATH = Path("/local/scripts/clone/clone_manager.py")
...
class WikiEngineTests(unittest.TestCase):
    ...
    def test_disabled_mode_is_safe_and_structured(self):
        ...
        self.assertIn("agents/private/shared/wiki/", Path("/local/.gitignore").read_text(encoding="utf-8"))
    ...
    def test_clone_manager_mounts_shared_wiki_only_when_enabled(self):
        ...
        with patch.object(CLONE_MANAGER, "_ensure_node_log_topology", lambda *args, **kwargs: None), \
             ...
             patch.object(CLONE_MANAGER, "SHARED_SCRIPTS_ROOT", root / "scripts"), \
             patch.object(CLONE_MANAGER, "SHARED_PLUGINS_ROOT", root / "plugins"), \
             patch.object(CLONE_MANAGER, "SHARED_WIKI_ROOT", root / "shared" / "wiki"):
