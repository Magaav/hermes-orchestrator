...
AGENTS_PRIVATE_ROOT = Path(os.getenv("HERMES_AGENTS_PRIVATE_ROOT", str(AGENTS_ROOT / "private")))
AGENTS_PUBLIC_ROOT = Path(os.getenv("HERMES_AGENTS_PUBLIC_ROOT", str(AGENTS_ROOT / "public")))
AGENTS_SHARED_ROOT = Path(
    os.getenv("HERMES_AGENTS_SHARED_ROOT", str(AGENTS_PRIVATE_ROOT / "shared"))
)
SHARED_WIKI_ROOT = Path(os.getenv("HERMES_SHARED_WIKI_ROOT", str(AGENTS_SHARED_ROOT / "wiki")))
...
LEGACY_WORKSPACE_ROOT = Path("/local/workspace")
PARENT_WORKSPACE_ROOT = LEGACY_WORKSPACE_ROOT
...
LEGACY_SHARED_ROOT = AGENTS_ROOT / "shared"
LEGACY_SHARED_WIKI_ROOT = LEGACY_SHARED_ROOT / "wiki"
LEGACY_PUBLIC_PLUGINS_ROOT = Path("/local/plugins")
LEGACY_PUBLIC_SCRIPTS_ROOT = Path("/local/scripts")
LEGACY_CRONS_ROOT = Path("/local/crons")
LEGACY_PLUGINS_MEMORY_ROOT = LEGACY_PUBLIC_PLUGINS_ROOT / "memory"
LEGACY_TMP_ROOT = Path("/local/tmp")
SHARED_PLUGINS_ROOT = Path(
    str(
        os.getenv("HERMES_PUBLIC_PLUGINS_ROOT", "")
        or os.getenv("HERMES_PLUGINS_ROOT", "")
        or (AGENTS_PUBLIC_ROOT / "plugins")
    )
)
SHARED_SCRIPTS_ROOT = Path(
    str(
        os.getenv("HERMES_PUBLIC_SCRIPTS_ROOT", "")
        or os.getenv("HERMES_SCRIPTS_ROOT", "")
        or (AGENTS_PUBLIC_ROOT / "scripts")
    )
)
...
# Canonical shared Discord ACL/users table now lives in the shared plugin root.
# Keep variable name stable to avoid touching unrelated call sites.
NODE_WORKSPACE_DISCORD_USERS_DB_IN_CONTAINER = "/local/plugins/discord/discord_users.json"
...
def _ensure_public_root_dir(target: Path, legacy_source: Path) -> None:
    ...
    if (
        ...
        and legacy_source.is_dir()
        and not legacy_source.is_symlink()
        and legacy_source.resolve() != target.resolve()
        and _dir_has_entries(legacy_source)
    ):
        _sync_dir(legacy_source, target, delete=False)


def _ensure_legacy_alias_link(alias_path: Path, canonical_target: Path) -> None:
    ...
    if alias_path.is_symlink():
        try:
            if alias_path.resolve() == canonical_target.resolve():
                return
        except Exception:
            pass
        _remove_path(alias_path)
    elif alias_path.exists():
        if alias_path.is_dir() and _dir_has_entries(alias_path):
            _sync_dir(alias_path, canonical_target, delete=False)
        _remove_path(alias_path)

    os.symlink(str(canonical_target), str(alias_path))


def _cleanup_legacy_topology_roots() -> None:
    for legacy in (LEGACY_TMP_ROOT, LEGACY_WORKSPACE_ROOT, LEGACY_CRONS_ROOT):
        if legacy.is_symlink():
            _remove_path(legacy)
            continue
        if not legacy.exists() or not legacy.is_dir():
            continue
        # Keep non-empty legacy roots for operator inspection/migration.
        if _dir_has_entries(legacy):
            # Special case: remove stale /local/workspace/wiki links from old
            # wiki bootstrap behavior so /local/workspace stops reappearing.
            stale_wiki = legacy / "wiki"
            if stale_wiki.is_symlink():
                _remove_path(stale_wiki)
            if _dir_has_entries(legacy):
                continue
        try:
            legacy.rmdir()
...
def _ensure_memory_root_layout() -> None:
    ...
    legacy_candidates = [LEGACY_PLUGINS_MEMORY_ROOT, LEGACY_MEMORY_ROOT]
    for legacy in legacy_candidates:
        ...
        if legacy.is_dir() and _dir_has_entries(legacy):
            _sync_dir(legacy, target, delete=False)
        _remove_path(legacy)


def _migrate_legacy_shared_wiki() -> None:
    if LEGACY_SHARED_WIKI_ROOT == SHARED_WIKI_ROOT:
        return
    if LEGACY_SHARED_WIKI_ROOT.exists():
        SHARED_WIKI_ROOT.parent.mkdir(parents=True, exist_ok=True)
        if not SHARED_WIKI_ROOT.exists():
            try:
                shutil.move(str(LEGACY_SHARED_WIKI_ROOT), str(SHARED_WIKI_ROOT))
            except Exception:
                _sync_dir(LEGACY_SHARED_WIKI_ROOT, SHARED_WIKI_ROOT, delete=False)
                _remove_path(LEGACY_SHARED_WIKI_ROOT)
        else:
            if LEGACY_SHARED_WIKI_ROOT.is_dir() and _dir_has_entries(LEGACY_SHARED_WIKI_ROOT):
                _sync_dir(LEGACY_SHARED_WIKI_ROOT, SHARED_WIKI_ROOT, delete=False)
            _remove_path(LEGACY_SHARED_WIKI_ROOT)
    if LEGACY_SHARED_ROOT.exists() and LEGACY_SHARED_ROOT != AGENTS_SHARED_ROOT:
        try:
            if LEGACY_SHARED_ROOT.is_dir() and not _dir_has_entries(LEGACY_SHARED_ROOT):
...
def _migrate_legacy_crons_root() -> None:
    if LEGACY_CRONS_ROOT == SHARED_CRONS_ROOT:
        return
    if not LEGACY_CRONS_ROOT.exists():
        return
    if LEGACY_CRONS_ROOT.is_symlink():
        _remove_path(LEGACY_CRONS_ROOT)
        return
    if not LEGACY_CRONS_ROOT.is_dir():
        _remove_path(LEGACY_CRONS_ROOT)
        return
    SHARED_CRONS_ROOT.mkdir(parents=True, exist_ok=True)
    if _dir_has_entries(LEGACY_CRONS_ROOT):
        _sync_dir(LEGACY_CRONS_ROOT, SHARED_CRONS_ROOT, delete=False)
    _remove_path(LEGACY_CRONS_ROOT)
...
def _ensure_dirs() -> None:
    ...
    AGENTS_PRIVATE_ROOT.mkdir(parents=True, exist_ok=True)
    AGENTS_PUBLIC_ROOT.mkdir(parents=True, exist_ok=True)
    AGENTS_SHARED_ROOT.mkdir(parents=True, exist_ok=True)
    ...
    _ensure_public_root_dir(SHARED_SCRIPTS_ROOT, LEGACY_PUBLIC_SCRIPTS_ROOT)
    _ensure_public_root_dir(SHARED_PLUGINS_ROOT, LEGACY_PUBLIC_PLUGINS_ROOT)
    _ensure_legacy_alias_link(LEGACY_PUBLIC_SCRIPTS_ROOT, SHARED_SCRIPTS_ROOT)
    _ensure_legacy_alias_link(LEGACY_PUBLIC_PLUGINS_ROOT, SHARED_PLUGINS_ROOT)
    _migrate_legacy_shared_wiki()
    _migrate_legacy_crons_root()
    _ensure_memory_root_layout()
    _cleanup_legacy_topology_roots()
...
def _build_node_runtime_contract_text(clone_name: str, env: Dict[str, str]) -> str:
    ...
    lines = [
        ...
        "- Shared container aliases: /local/plugins and /local/scripts",
        ...
        "",
        "## Plugin Governance",
        "- Workers must treat /local/plugins and /local/scripts (hosted by /local/agents/public/*) as orchestrator-managed assets.",
...
def _ensure_worker_shared_mount_links(clone_root: Path, clone_name: str) -> None:
    """Ensure worker node host tree has mount points for shared roots."""
    ...
    for rel in ("scripts", "plugins", "cron"):
        path = clone_root / rel
        if path.is_symlink():
            _remove_path(path)
...
def _sync_discord_runtime_layout(clone_root: Path, clone_name: str = "") -> None:
    """Use shared plugin runtime state and remove legacy workspace mirrors."""
    ...
    # Remove stale legacy mirrors. Compatibility is handled by path fallbacks.
...
def _link_shared_plugins_for_clone(clone_root: Path, clone_name: str) -> None:
    """Symlink shared discord plugin into clone plugins directory (not copied).
    ...
    with a symlink to the shared plugins root.
    """
    plugins_dir = clone_root / "plugins"
    plugins_dir.mkdir(parents=True, exist_ok=True)

    shared_plugins = SHARED_PLUGINS_ROOT
...
def _setup_orchestrator_baremetal(clone_name: str, clone_root: Path, env: Dict[str, str], env_path: Path) -> Dict[str, Any]:
    ...
    - Uses shared host assets via symlinks: /local/agents/public/{plugins,scripts} and /local/agents/private/crons/orchestrator
...
def _parent_skills_source() -> Path:
    # Canonical source of truth is /local/agents/private/skills. We keep
    ...
    raise CloneManagerError(
        ...
        "(expected /local/agents/private/skills or orchestrator .hermes/skills)."
    )
...
def _normalize_clone_skills_layout(clone_root: Path, *, containerized: bool) -> None:
    ...
    - Canonical shared skills dir: /local/agents/private/skills
    - Node-local /local/.hermes/skills becomes a symlink to ../skills
    - Host orchestrator uses node skills symlink -> /local/agents/private/skills
...
def _prepare_clone_filesystem(clone_name: str, clone_root: Path, env: Dict[str, str], env_path: Path) -> Dict[str, Any]:
    ...
    # Keep workspace/data as node-local mutable runtime state.
...
def _action_backup(clone_name: str | None, *, backup_all: bool) -> Dict[str, Any]:
    ...
    if backup_all:
        ...
        if LEGACY_CRONS_ROOT.exists():
            global_candidates.append(LEGACY_CRONS_ROOT)
    else:
...
def _collect_restore_source(restore_root: Path) -> Dict[str, Any]:
    ...
    private_root = agents_root / "private"
    ...
    return {
        ...
        "env_files": env_files,
        "node_dirs": node_dirs,
        "private_root_path": private_root if private_root.exists() else None,
...
def _action_restore(restore_path: str) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="horc-restore-", dir=str(BACKUPS_ROOT)) as tmp:
        ...
        private_root_path: Path | None = source.get("private_root_path")
        ...
        for node, node_src in sorted(node_dirs.items()):
            ...
            try:
                ...
                restored_mode = 2
            if restored_mode == 1:
                _set_symlink(node_dst / "scripts", SHARED_SCRIPTS_ROOT)
                _set_symlink(node_dst / "plugins", SHARED_PLUGINS_ROOT)
        ...
        if isinstance(private_root_path, Path) and private_root_path.exists():
            AGENTS_PRIVATE_ROOT.mkdir(parents=True, exist_ok=True)
            _sync_dir(private_root_path, AGENTS_PRIVATE_ROOT, delete=False)
            restored_private_root = str(AGENTS_PRIVATE_ROOT)
    ...
    return {
        ...
        "restored_private_root": restored_private_root,
