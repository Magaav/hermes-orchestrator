...
class BridgeSettings:
    ...
    def from_env(cls, plugin_root: Path) -> "BridgeSettings":
        ...
        legacy_state_dir = (repo_root / "plugins" / "private" / "hermes-space-ui").resolve()
        state_dir = Path(
            os.getenv(
                "HERMES_SPACE_UI_STATE_DIR",
                str(repo_root / "plugins" / "hermes-space-ui" / "state"),
            )
        ).resolve()
        if state_dir == legacy_state_dir:
            raise ValueError(
                "Legacy hermes-space-ui state path is no longer supported: "
                f"{legacy_state_dir}. Use /local/plugins/hermes-space-ui/state instead."
            )
        if legacy_state_dir.exists():
            try:
                legacy_has_files = any(legacy_state_dir.iterdir())
            except OSError:
                legacy_has_files = True
            if legacy_has_files:
                raise ValueError(
                    "Legacy hermes-space-ui state was detected at "
                    f"{legacy_state_dir}. Migrate or delete it before using "
                    f"{state_dir}."
                )
        return cls(
            plugin_root=plugin_root,
            repo_root=repo_root,
            host=str(os.getenv("HERMES_SPACE_UI_HOST", "127.0.0.1") or "127.0.0.1"),
            port=int(str(os.getenv("HERMES_SPACE_UI_PORT", "8790") or "8790")),
            token=str(os.getenv("HERMES_SPACE_UI_TOKEN", "") or "").strip(),
            horc_path=str(os.getenv("HERMES_SPACE_UI_HORC", str(default_horc)) or default_horc),
            agents_root=agents_root,
            state_dir=state_dir,
            timeout_sec=float(str(os.getenv("HERMES_SPACE_UI_TIMEOUT_SEC", "120") or "120")),
            space_agent_url=str(os.getenv("SPACE_AGENT_URL", "") or "").strip(),
            space_agent_repo=str(
                os.getenv("SPACE_AGENT_REPO", "https://github.com/agent0ai/space-agent")
                or "https://github.com/agent0ai/space-agent"
            ).strip(),
            agent_root=Path(
                os.getenv(
                    "HERMES_SPACE_UI_AGENT_ROOT",
                    str(repo_root / "agents" / "nodes" / "orchestrator" / "hermes-agent"),
                )
            ).resolve(),
            agent_env_path=Path(
                os.getenv(
                    "HERMES_SPACE_UI_AGENT_ENV",
                    str(repo_root / "agents" / "envs" / "orchestrator.env"),
                )
            ).resolve(),
            agent_python=Path(
                os.getenv(
                    "HERMES_SPACE_UI_AGENT_PYTHON",
                    str(
                        repo_root
                        / "agents"
                        / "nodes"
...
class OrchestratorClient:
    ...
    def start_drop_to_copy_task(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        target_node = str(
            ...
            or os.getenv("HERMES_SPACE_UI_DROP_TO_COPY_NODE", "orchestrator")
            or "orchestrator"
        ).strip()
    ...
    def _run_drop_to_copy_task(self, task_id: str, node: str, prompt: str) -> None:
        ...
        self.task_store.finish(
            ...
            result={
                ...
                "node_id": node,
                "model_request": {
                    "model": os.getenv("HERMES_SPACE_UI_DROP_TO_COPY_MODEL", "gpt-5.5"),
    ...
    def _drop_to_copy_run_options(self) -> dict[str, Any]:
        model = str(os.getenv("HERMES_SPACE_UI_DROP_TO_COPY_MODEL", "gpt-5.5") or "gpt-5.5").strip()
        reasoning_effort = str(
            os.getenv("HERMES_SPACE_UI_DROP_TO_COPY_REASONING_EFFORT", "xhigh")
            or "xhigh"
        ).strip()
    ...
    def _api_server_url_for_node(self, node_id: str) -> str:
        ...
        env_key = f"HERMES_SPACE_UI_API_SERVER_{node.upper().replace('-', '_')}_URL"
        ...
        raise BridgeError(
            ...
            details={
                "node_id": node,
                "env": env_key,
                "fallback_env": "HERMES_SPACE_UI_API_SERVER_URL",
            },
        )

    def _api_server_key_for_node(self, node_id: str) -> str:
        node = validate_node_id(node_id)
        env_key = f"HERMES_SPACE_UI_API_SERVER_{node.upper().replace('-', '_')}_KEY"
        explicit = str(os.getenv(env_key, "") or "").strip()
        if explicit:
            return explicit
    ...
    def run_chat_completion(self, payload: dict[str, Any]) -> dict[str, Any]:
        node = str(payload.get("target_node") or os.getenv("HERMES_SPACE_UI_DEFAULT_API_NODE", "orchestrator"))
...
class SpaceUIHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args: Any) -> None:
        if os.getenv("HERMES_SPACE_UI_ACCESS_LOG", "").lower() in {"1", "true", "yes", "on"}:
    ...
    def _health(self) -> None:
        ...
        payload = {
            ...
            "space_agent_url_configured": bool(settings.space_agent_url),
            "space_agent_repo": settings.space_agent_repo,
    ...
    def _capabilities(self) -> None:
        ...
        payload = {
            ...
            "space_agent": {
                "repo": settings.space_agent_repo,
                "role": "mutable_visual_workspace",
            },
    ...
    def _require_auth(self) -> None:
        ...
        if not is_authorized(self.headers, settings.token):
            raise BridgeError(
                "unauthorized",
                "Missing or invalid Hermes Space UI token.",
...
def is_space_ui_generated_env(path: Path, node_id: str) -> bool:
    try:
        ...
        return False
    return (
        any(line.strip() == "# Generated by hermes-space-ui." for line in lines)
...
def node_activity_snapshot(settings: BridgeSettings, node_id: str) -> dict[str, Any]:
    ...
    now_ts = time.time()
    try:
        configured_window = float(str(os.getenv("HERMES_SPACE_UI_LLM_ACTIVE_WINDOW_SEC", "30") or "30"))
    except ValueError:
...
def host_resources_snapshot(settings: BridgeSettings) -> dict[str, Any]:
    ...
    return {
        ...
        "timestamp": utc_now(),
        "source": "hermes-space-ui bridge host",
        "host": {
