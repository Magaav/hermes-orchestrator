#!/usr/bin/env python3
"""Bootstrap the Hermes-native Canva plugin into a node runtime."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bootstrap the Canva project plugin into Hermes.")
    parser.add_argument("--env-file", required=True)
    parser.add_argument("--config-file", default="")
    parser.add_argument("--plugin-source", default="/local/plugins/public/native/canva")
    parser.add_argument("--plugin-name", default="canva")
    ...
    enabled = _is_truthy(env.get("PLUGIN_CANVA", ""))
    missing = [key for key in ("CANVA_REFRESH_TOKEN", "CANVA_CLIENT_ID", "CANVA_CLIENT_SECRET") if not str(env.get(key, "") or "").strip()]
    ...
    if enabled:
        if missing:
            print(json.dumps({"ok": False, "enabled": True, "missing": missing, "error": "Missing Canva credentials"}))
            return 1
        ...
        print(json.dumps({"ok": True, "enabled": True, "changed": changed, "plugin_synced": synced, "plugin_target": str(plugin_target), "config_file": str(config_file), "env_file": str(env_file)}))
        return 0
    ...
    if removed:
        _save_config(config_file, config)
        changed = True
    print(json.dumps({"ok": True, "enabled": False, "changed": changed, "plugin_synced": False, "plugin_target": str(plugin_target), "config_file": str(config_file), "env_file": str(env_file)}))
    return 0
