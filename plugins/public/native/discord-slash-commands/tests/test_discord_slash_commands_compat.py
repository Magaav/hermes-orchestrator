...
COMPAT_PATH = Path("/local/plugins/public/native/discord-slash-commands/compat.py")


def _load_compat():
    spec = importlib.util.spec_from_file_location("discord_slash_commands_compat_test", COMPAT_PATH)
...
def test_ensure_discord_slash_runtime_syncs_hooks(monkeypatch, tmp_path):
    compat = _load_compat()
    ...
    payload = compat.ensure_discord_slash_runtime()
    ...
    assert "metricas" in registry["slash_bridge"]["force_bridge_commands"]
    assert registry["native_overrides"]["metricas"]["enabled"] is True
