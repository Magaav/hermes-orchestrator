...
from discord_slash_commands_env_bootstrap import main
...
def test_bootstrap_disabled_removes_from_enabled(monkeypatch, tmp_path, capsys):
    ...
    env_file.write_text("PLUGIN_DISCORD_SLASH_COMMANDS=false\n", encoding="utf-8")
    config_file.write_text(yaml.safe_dump({"plugins": {"enabled": ["discord-slash-commands", "other"]}}), encoding="utf-8")
...
def test_bootstrap_enabled_syncs_plugin_and_updates_env(monkeypatch, tmp_path, capsys):
    ...
    env_file.write_text("PLUGIN_DISCORD_SLASH_COMMANDS=true\n", encoding="utf-8")
