...
from wiki_engine_env_bootstrap import main


def _plugin_source(tmp_path: Path) -> Path:
    plugin = tmp_path / "wiki-engine-source"
    plugin.mkdir()
    (plugin / "plugin.yaml").write_text("name: wiki-engine\n", encoding="utf-8")
...
def test_bootstrap_disabled_removes_from_enabled(monkeypatch, tmp_path, capsys):
    ...
    env_file.write_text("PLUGIN_WIKI_ENGINE=false\n", encoding="utf-8")
    config_file.write_text(yaml.safe_dump({"plugins": {"enabled": ["wiki-engine", "other"]}}), encoding="utf-8")

    monkeypatch.setattr(
        sys,
        "argv",
        [
            "wiki_engine_env_bootstrap.py",
            "--env-file",
            str(env_file),
            "--config-file",
            str(config_file),
...
def test_bootstrap_enabled_syncs_plugin_and_updates_env(monkeypatch, tmp_path, capsys):
    ...
    env_file.write_text("PLUGIN_WIKI_ENGINE=true\n", encoding="utf-8")
    ...
    cfg = yaml.safe_load(config_file.read_text(encoding="utf-8"))
    assert "wiki-engine" in cfg["plugins"]["enabled"]
    assert (config_file.parent / "plugins" / "wiki-engine" / "plugin.yaml").exists()
