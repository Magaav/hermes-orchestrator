...
# Canonical plugin roots (hard-switch contract).
DISCORD_PLUGIN_ROOT="${HERMES_DISCORD_PLUGIN_DIR:-/local/plugins/public/discord}"
HERMES_CORE_PLUGIN_ROOT="${HERMES_CORE_PLUGIN_DIR:-/local/plugins/public/hermes-core}"
DISCORD_PRIVATE_ROOT="${HERMES_DISCORD_PRIVATE_DIR:-/local/plugins/private/discord}"

if [[ ! -d "$DISCORD_PLUGIN_ROOT" ]]; then
  echo "[error] discord public plugin root not found: $DISCORD_PLUGIN_ROOT" >&2
  exit 1
fi
if [[ ! -d "$HERMES_CORE_PLUGIN_ROOT" ]]; then
  echo "[error] hermes-core public plugin root not found: $HERMES_CORE_PLUGIN_ROOT" >&2
  exit 1
fi
if [[ ! -d "$DISCORD_PRIVATE_ROOT" ]]; then
  echo "[error] discord private runtime root not found: $DISCORD_PRIVATE_ROOT" >&2
  exit 1
fi

# Explicitly expose clone code root for patch scripts.
export HERMES_AGENT_ROOT="${HERMES_AGENT_ROOT:-/local/hermes-agent}"
export HERMES_DISCORD_PLUGIN_DIR="$DISCORD_PLUGIN_ROOT"
export HERMES_CORE_PLUGIN_DIR="$HERMES_CORE_PLUGIN_ROOT"
export HERMES_DISCORD_PRIVATE_DIR="$DISCORD_PRIVATE_ROOT"
...
WIKI_ENGINE_SCRIPT="$HERMES_CORE_PLUGIN_ROOT/scripts/wiki_engine.py"
if [[ -f "$WIKI_ENGINE_SCRIPT" ]]; then
  NODE_ROOT_FOR_WIKI="${HERMES_NODE_ROOT:-}"
  if [[ -z "$NODE_ROOT_FOR_WIKI" && -n "${HERMES_HOME:-}" ]]; then
    case "${HERMES_HOME}" in
      */.hermes)
        NODE_ROOT_FOR_WIKI="${HERMES_HOME%/.hermes}"
        ;;
    esac
  fi
  if [[ -z "$NODE_ROOT_FOR_WIKI" && -n "${NODE_NAME:-}" && -d "/local/agents/nodes/${NODE_NAME}" ]]; then
    NODE_ROOT_FOR_WIKI="/local/agents/nodes/${NODE_NAME}"
  fi
  if [[ -z "$NODE_ROOT_FOR_WIKI" ]]; then
    NODE_ROOT_FOR_WIKI="/local"
  fi
  run_step "wiki_bootstrap" \
    "$PYTHON_BIN" "$WIKI_ENGINE_SCRIPT" bootstrap --node-name "${NODE_NAME:-}" --node-root "$NODE_ROOT_FOR_WIKI" --repair || FAILED=1
fi
