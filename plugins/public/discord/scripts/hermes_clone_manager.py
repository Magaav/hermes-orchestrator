...
SHARED_PLUGINS_ROOT = Path(
    str(os.getenv("HERMES_PUBLIC_PLUGINS_ROOT", "") or "").strip() or "/local/plugins/public"
)
PRIVATE_PLUGINS_ROOT = Path(
    str(os.getenv("HERMES_PRIVATE_PLUGINS_ROOT", "") or "").strip() or "/local/plugins/private"
)
...
def _bootstrap_camofox_for_clone(clone_name: str, env_path: Path) -> Dict[str, Any]:
    ...
    if camofox_bootstrap_script.exists():
        python_candidates = [
            Path("/local/hermes-agent/.venv/bin/python3"),
...
def _parent_skills_source() -> Path:
    # Canonical source of truth is /local/skills, with HERMES_HOME/skills as
    # compatibility source for migration safety.
    candidates = [
        Path("/local/skills"),
...
def _normalize_clone_skills_layout(clone_root: Path) -> None:
    ...
    # Drop legacy workspace compatibility link if present.
...
def _seed_clone_workspace_integrations(clone_root: Path) -> Dict[str, bool]:
    ...
    Shared plugins are mounted from /local/plugins/public (not copied into
...
def _extract_state_mode(env: Dict[str, str]) -> int:
    ...
    if value not in STATE_LABELS:
        raise CloneManagerError(
            f"invalid CLONE_STATE value: {value}. Expected 1 (fresh), 2 (seed_from_parent_snapshot), or 3 (seed_from_backup)."
        )
    return value
...
def _build_docker_run_cmd(
    ...
    env_path: Path,
    image: str,
    shared_public_plugins_root: Path,
    private_plugins_root: Path,
    *,
    ...
    cmd = [
        ...
        f"{clone_root}:/local",
        "-v",
        f"{shared_public_plugins_root}:/local/plugins/public:ro",
        "-v",
...
def _action_start(clone_name: str, image: str) -> Dict[str, Any]:
    ...
    shared_discord_plugin_root = _discord_plugin_dir(require_exists=True)
    prestart_script = shared_discord_plugin_root / "scripts" / "prestart_reapply.sh"
    if not prestart_script.exists():
        raise CloneManagerError(
            f"discord public plugin missing required prestart script: {prestart_script}"
        )
    shared_public_plugins_root = shared_discord_plugin_root.parent
    private_plugins_root = PRIVATE_PLUGINS_ROOT
    private_plugins_root.mkdir(parents=True, exist_ok=True)
    DEFAULT_DISCORD_PRIVATE_ROOT.mkdir(parents=True, exist_ok=True)
