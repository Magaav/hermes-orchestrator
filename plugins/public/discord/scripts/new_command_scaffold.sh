...
def main() -> int:
    ...
    root = Path(os_mod.getenv("HERMES_DISCORD_PLUGIN_DIR", "/local/plugins/public/discord")).resolve()
    private_root = Path(
        os_mod.getenv("HERMES_DISCORD_PRIVATE_DIR", "/local/plugins/private/discord")
    ).resolve()
    core_prestart = Path(
        os_mod.getenv("HERMES_CORE_PLUGIN_DIR", "/local/plugins/public/hermes-core")
    ).resolve() / "scripts" / "prestart_reapply.sh"
    if not core_prestart.exists():
        core_prestart = root / "scripts" / "prestart_reapply.sh"
    commands_dir = private_root / "commands"
