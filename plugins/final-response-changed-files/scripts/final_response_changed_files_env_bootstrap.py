#!/usr/bin/env python3
"""Bootstrap the final-response-changed-files project plugin into a node runtime."""
...
CANONICAL_ENABLE_KEY = "PLUGIN_FINAL_RESPONSE_FILES_CHANGED"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bootstrap the final-response-changed-files project plugin into Hermes.")
    parser.add_argument("--env-file", required=True)
    parser.add_argument("--config-file", default="")
    parser.add_argument("--plugin-source", default="/local/plugins/public/native/final-response-changed-files")
    parser.add_argument("--plugin-name", default="final-response-changed-files")
    ...
    enabled = _is_truthy(env.get(CANONICAL_ENABLE_KEY, ""))
    ...
    if enabled:
        changed = _upsert_env_value(env_file, CANONICAL_ENABLE_KEY, "true") or changed
