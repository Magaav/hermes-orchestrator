...
def live_client(client: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    client_field: dict[str, Any] = {"type": "string", "maxLength": 120}
    if client_id:
        client_field.update({"enum": [client_id], "default": client_id})
    ...
    result = [contracts.capability({
        ...
        "input": {"type": "object", "properties": {"client": client_field}, "additionalProperties": False},
    })]
    if "control.widget.open" in capabilities:
        ...
        result.append(contracts.capability({
            ...
            "mode": "write", "conflicts": ["client:{client}"], "proof": ["client.ack"], "detail": binding,
            "input": {"type": "object", "required": ["widget"], "properties": {"client": {**client_field, "minLength": 1}, "widget": widget_field, "wait_sec": {"type": "number", "minimum": 0, "maximum": 20}}, "additionalProperties": False},
        }))
    if "control.browser.navigate" in capabilities:
        result.append(contracts.capability({
            ...
            "mode": "write", "conflicts": ["client:{client}"], "proof": ["client.ack", "browser.url"], "detail": binding,
            "input": {"type": "object", "required": ["client", "url"], "properties": {"client": {**client_field, "minLength": 1}, "url": {"type": "string", "minLength": 1, "maxLength": 2000, "pattern": "^https://"}, "wait_sec": {"type": "number", "minimum": 0, "maximum": 20}}, "additionalProperties": False},
        }))
    return result
