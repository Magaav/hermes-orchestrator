...
| V6 model-native agent kernel | repository/Electron, trajectory dataflow, and exact-repeat Windows read procedure memory production verified; browser default | `MASTER_FRONTIER_V6.md`; `python3 ../../tools/context/prove-master-frontier-v6-trajectory.py`; `python3 ../../tools/context/prove-master-frontier-v6-procedure-memory.py`; V6 proof ladder | Production run `wa_run_bea1f2a036714c98993db76d58d3aea0` proved the repository trajectory lane. Exact-repeat Windows-list runs then calibrated twice and replayed with a fresh native proof at zero provider calls/tokens in `wa_run_7e7aae0a2b7e4e31bfab08e41b8f18d5`. Procedure memory is read-only, exact-text, proof-bound, and separately disabled by `MF_V6_PROCEDURE_MEMORY=0`; V5 remains an immediate protocol rollback lane. |
| Cross-journey product-readiness evaluator | evaluator verified; product not ready | `python3 ../../tools/context/evaluate-wasm-agent-product-readiness.py --validate-only`; artifact-only evaluator; latest JSON/Markdown report | Composes `repository-agent` and `android-voice-agent` registered promises into schema `wasm-agent.product-readiness.result.v1`. Default execution reads artifacts only; `--run <journey-id>` explicitly serializes child promises. |
...
| Native release feed | verified | `public/native/releases/latest.json`; `native/windows/release/VERIFY.json`; `reports/windows/latest/windows-release-feed-check.json` | Current metadata publishes Windows build `win-x64-20260826T132817Z`, matching verified installer SHA `284cac6c5c5d371b65ee23658c325754a792dc73baed5f9c5aaab9793a1386f5`; the production origin returned the same build after deployment. |
...
| Getter-safe CDP runtime inspection | implemented-unverified | `node ../../native/windows/ops/desktop/windows-cdp-runtime-inspect.test.js`; focused V6 client/capability tests | Candidate read-only fallback after semantic DOM inspection is insufficient. Returns bounded property descriptors, DOM/runtime correlation, revision, and a short-lived handle; production activation and comparative runtime proof remain required. |
| Unified live-client registry | production verified for bounded client and Windows control | `python3 tests/live_clients.test.py`; production authority and V6 client-UI proofs; authenticated `GET /native/control/clients` | Normalizes live/stale PWA, Electron, and Android/Kotlin clients with bounded explicitly declared capabilities. Browser automation belongs exclusively to native CDP realms. |
| Master:frontier full-power client execution | installed runtime and Windows environment adapter verified | `master-frontier-full-power-client-source`; `reports/context/latest/master-frontier-full-power-client-runtime.json`; `reports/context/latest/master-frontier-windows-environment-canary.json` | The chat receives an always-visible compact `client.environment.inspect` capability. Its renderer remains the primary UI client while the sibling installed desktop kernel separately owns UIA and current-user PowerShell capabilities, each pinned to its exact device ID. Production V6 executed the environment inspection, returned a nonempty model answer with exact usage, changed no files, and closed a verified terminal anchor. Authority remains authenticated, audited, size/time bounded, not Administrator/SYSTEM elevation, and unavailable to plain PWA or Android clients. |
| Windows companion token + desktop automation | installed runtime verified with privilege limits | `npm --prefix ../../native/windows/src run test:companion-desktop-source`; `windows-companion-desktop-source`; `reports/windows/latest/windows-companion-desktop-runtime.json` | Build `win-x64-20260826T153423Z` uses event-time pointer coordinates plus drag-session IDs, rejects stale updates, preserves size, and excludes header buttons from drag capture. Physical input proved exact same-display mouse sync, zero post-release drift under an adversarial cursor move, compact/full-size restoration, and working Sessions, Settings, and Close. Administrator/SYSTEM/UAC remain unsupported. |
| Windows final NSIS/app.asar package | verified | `native/windows/release/VERIFY.json` | Build `win-x64-20260826T153423Z` passed final extracted-NSIS verification and excludes source tests. Installer SHA is `037601910614c095c68f0a0f12da99457d2b9c056161f4273b1a55c99ad386ca`; app.asar SHA is `531c868ec9698aaaf07cee88282c2ea3943284e8f97af0ff73040dbf84b3df76`. |
| Windows installed-app behavior | current package/session verified; strongest full-close continuity pending | `reports/windows/latest/windows-update-reconnect.json`; `reports/windows/latest/windows-companion-desktop-runtime.json` | The same installed client transitioned to build `win-x64-20260826T153423Z`; `electron.original-fs` proved the exact expected `app.asar` SHA, and the post-update session has `wa_uid`, durable expiration metadata, authenticated `/auth/session`, the production companion route, and no frontend fatal error. A separately correlated full application close/reopen receipt remains required only for the strongest login-persistence claim. |
...
Non-conceptual V6 finalization is implemented locally as a generic
claim-to-receipt gate. Runtime code no longer selects the Windows UIA primitive
from Windows-specific prompt words; the head selects a declared capability and
must cite the successful viewed receipt and its capability-declared proof. The
exact prior Windows-control and self-diagnosis prompts are regression fixtures
only. Deployment and an unchanged production rerun remain unverified.
...
| `MASTER_FRONTIER_V6.md` | Opt-in model-native agent kernel, compact projection, capability adapters, parallel scheduler, persistence, commentary, and production promotion evidence | Read before V6 protocol, controller, adapter, MCP, execution, context-efficiency, or promotion changes. |
...
| Product readiness, one explicit journey | `python3 tools/context/evaluate-wasm-agent-product-readiness.py --run <repository-agent|android-voice-agent>` |
...
- Authenticated remote `apply_windows_update` control is an explicit automatic approval: the renderer forwards both `automatic: true` and `applyApproved: true`, while the manual Check Update flow remains interactive.
