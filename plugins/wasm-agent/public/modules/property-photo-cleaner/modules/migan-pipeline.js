...
export async function inpaintMigan(session, frame, diagnostics = null) {
  ...
  diagnostics?.onPhase?.("crop-planned", { crop });
  ...
  diagnostics?.onPhase?.("pixels-resized", { pixels: MODEL_SIZE * MODEL_SIZE });
  ...
  diagnostics?.onPhase?.("arrays-packed", {
    imageBytes: rgb.byteLength,
  ...
  diagnostics?.onPhase?.("tensors-created", {});
  let outputs;
  try {
    diagnostics?.onPhase?.("inference-starting", {
      imageBytes: rgb.byteLength,
      maskBytes: mask.byteLength
    });
    ...
    if (diagnostics) {
      diagnostics.inferenceMs = Math.round(performance.now() - inferenceStartedAt);
      diagnostics.onPhase?.("inference-complete", { inferenceMs: diagnostics.inferenceMs });
    }
    const output = outputs.result || Object.values(outputs)[0];
    const readbackStartedAt = performance.now();
    const nchw = await output.toTypedArray();
    if (diagnostics) {
      diagnostics.readbackMs = Math.round(performance.now() - readbackStartedAt);
      diagnostics.onPhase?.("output-ready", { readbackMs: diagnostics.readbackMs });
    ...
    if (diagnostics) diagnostics.quality = quality;
