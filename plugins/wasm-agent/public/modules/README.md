...
`runtime-refresh.js` owns the production cloud-module refresh transaction. The
model-facing `client.runtime.refresh` capability updates bounded service-worker
registrations, adds a cache-busting revision to the same-origin production URL,
and schedules a renderer replacement after its command receipt can be posted.
It never replaces Electron, preload, IPC, permissions, or other native code.

`../module-release.json` is generated by
`scripts/generate-module-release.mjs`. Its release ID is a SHA-256 digest over
the bounded boot/module graph. `app-loader.js` fetches that manifest with
`no-store` and applies the digest only to the selected app entry. The service
worker revalidates nested `/modules/` assets during reload while retaining its
cached response as the offline fallback, so child changes no longer require
manual parent cache-key edits.

Cloud module activation uses `scripts/deploy-cloud-modules.sh`: it generates
and checks the manifest before restarting production. The production systemd
unit independently runs the same `--check` as `ExecStartPre`, so a missing or
stale manifest fails closed even when the service is restarted directly.

`client.runtime.diagnose` starts a bounded observability lease and returns a
redacted snapshot of document readiness, aggregate errors/rejections,
performance counters, module-script counts, focused-control shape, viewport,
and recent interaction outcomes. Expanded logs and screenshots remain
pull-on-demand rather than default model context.
...
and retain a bounded diagnostic on the host. Interactive and remote opens apply
the visible window state before lazy mount so native-backed modules measure their
real laid-out viewport; a failed mount restores the minimized state.
