...
export function canCleanBatch(items, busy = false) {
  ...
  const included = items.filter((item) => item.included);
  return included.some((item) => ["ready", "failed"].includes(item.state))
    && included.every((item) => ["ready", "failed", "cleaned"].includes(item.state));
}

export function cleanedFilename(name, index = 0, outputExtension = "") {
  const safe = String(name || `photo-${index + 1}.jpg`).replace(/[\\/:*?"<>|]+/g, "-");
  const dot = safe.lastIndexOf(".");
  const stem = dot > 0 ? safe.slice(0, dot) : safe;
  const extension = outputExtension || (dot > 0 ? safe.slice(dot) : ".jpg");
