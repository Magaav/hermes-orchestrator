...
- Production shared-space voice controls are disabled by default during
  `/voice-lab` recovery. Set `HERMES_WASM_AGENT_SHARED_VOICE_ENABLED=1` only for
  deliberate production voice testing, and configure
  `HERMES_WASM_AGENT_VOICE_ICE_SERVERS_JSON` with TURN servers if either side
  cannot establish direct/STUN-only WebRTC audio.
