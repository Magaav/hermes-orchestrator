...
def main() -> int:
    ...
    if any(WORKSPACE.iterdir()) or any(RESULT.iterdir()):
    ...
    if args.execution in {"live-proof", "live"}:
        ...
        if task.get("schema") != "wasm-agent.safe-lab.fixture-task.v1" or task.get("model") != model["model"]:
        ...
        timed_out = False
        try:
            completed = subprocess.run(resolved, cwd=WORKSPACE, capture_output=True, text=True, timeout=timeout, check=False)
    ...
    (RESULT / "events.jsonl").write_text(json.dumps({"type": "lane.finished", "slot": args.slot, "status": status}) + "\n", encoding="utf-8")
