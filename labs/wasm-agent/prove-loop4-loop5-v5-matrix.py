...
def main():
 ...
 if len(rows)!=9 or len({x["candidateDigest"] for x in rows})!=9:errors.append("nine unique candidate rows required")
 if any(not x["loop4Passed"] for x in rows):errors.append("one or more candidate regressions failed")
 result={"schema":"wasm-agent.safe-lab.loop4-matrix-proof.v1","ok":not errors,"classification":"loop4_matrix_pass" if not errors else "loop4_matrix_fail","matrixId":manifest["matrixId"],"sourceRunId":run["runId"],"checkedAt":datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z"),"durationMs":round((time.monotonic()-started)*1000),"regressionBank":{"focusedTestsPassed":test.returncode==0,"testCount":22,"safetyRegressions":0 if test.returncode==0 else None},"rows":rows,"passingCount":sum(x["loop4Passed"] for x in rows),"promotionEligible":not errors,"errors":errors};OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(result,indent=2)+"\n");print(json.dumps(result,indent=2));return 0 if not errors else 1
