...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async def on_ready():
                ...
                logger.info("[%s] Syncing slash commands...", adapter_self.name)
                try:
            ...
            # Wait for ready
            # Wait for ready
            # Some bots in large guilds or with many commands take > 30s to sync.
            # Default to 120s for safer startup on slow connections.
    ...
    async def _handle_message(self, message: DiscordMessage) -> None:
        ...
        if not isinstance(message.channel, discord.DMChannel):
            ...
            # Check for role mention (e.g. <@&1490703816671363084> for @Orchestrator)
            bot_role_id = os.getenv("DISCORD_BOT_ROLE_ID", "1490703816671363084").strip()
            role_mention_pattern = f"<@&{bot_role_id}>"
            has_role_mention = role_mention_pattern in (message.content or "")

            if require_mention and not is_free_channel and not in_bot_thread:
                if self._client.user not in message.mentions and not has_role_mention:
                    return
            ...
            if self._client.user and self._client.user in message.mentions:
                ...
                message.content = message.content.replace(f"<@!{self._client.user.id}>", "").strip()
            if has_role_mention:
                message.content = message.content.replace(role_mention_pattern, "").strip()
