...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            if event.message_type == MessageType.PHOTO:
                ...
                # Use FIFO queue: get last event from queue for photo merging
            ...
            # Use FIFO queue
            self.enqueue_pending_message(session_key, event, interrupt=True)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # Use FIFO queue method instead of direct dict pop
            ...
            # Auto-trigger: check if there are more pending messages in FIFO queue
            # This handles messages that were queued during processing and after normal completion
            await asyncio.sleep(0)  # Yield to allow any async enqueues to complete
            auto_event = self.get_pending_message(session_key)
            if auto_event is not None:
                print(f"[{self.name}] 📨 Auto-triggering next queued message")
                new_task = asyncio.create_task(
                    self._process_message_background(auto_event, session_key)
                )
                try:
                    ...
                    if hasattr(new_task, "add_done_callback"):
    ...
    def _ensure_pending_queue(self, session_key: str) -> Deque[MessageEvent]:
        ...
        if session_key not in self._pending_messages:
            self._pending_messages[session_key] = deque()
        return self._pending_messages[session_key]
    ...
    def enqueue_pending_message(
        ...
        Args:
            ...
            event: Message event to queue
            interrupt: Whether this message should trigger an interrupt
            merge_photo: Whether to merge photo events (for photo bursts)
        """
        queue = self._ensure_pending_queue(session_key)
        
        # Handle photo burst merging
        ...
        # Set interrupt flag and add to queue
        if interrupt:
            event.interrupt = True
        queue.append(event)
    
    def get_pending_message(self, session_key: str) -> Optional[MessageEvent]:
        """Get the next message from the FIFO queue for a session.
        
        Returns the oldest event (FIFO order) and removes it from queue.
        Returns None if queue is empty or doesn't exist.
        """
