...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        self._channel_model_routing = self._load_channel_model_routing()
    ...
    def _resolve_turn_agent_config(self, user_message: str, model: str, runtime_kwargs: dict, source: Optional[SessionSource] = None) -> dict:
        ...
        try:
            # Highest-priority override: channel/thread-specific routing policy.
            if source is not None:
                channel_route = resolve_channel_turn_route(
                    user_message,
                    getattr(self, "_channel_model_routing", {}),
                    source,
                    primary,
                )
                if channel_route:
            ...
            # Fallback: generic cheap-vs-strong smart routing.
            return resolve_turn_route(user_message, getattr(self, "_smart_model_routing", {}), primary)
        except Exception:
            logger.exception("Turn route resolution failed; falling back to primary runtime")
    ...
    @staticmethod
    def _load_channel_model_routing() -> dict:
        """Load optional channel/thread-specific routing and guardrail config."""
        try:
            import yaml as _y
            cfg_path = _hermes_home / "config.yaml"
            if cfg_path.exists():
                with open(cfg_path, encoding="utf-8") as _f:
                    cfg = _y.safe_load(_f) or {}
                return cfg.get("channel_model_routing", {}) or {}
        except Exception:
            pass
        return {}
            ...
            if event.message_type == MessageType.PHOTO:
                ...
                if adapter:
                    if hasattr(adapter, "enqueue_pending_message"):
                        ...
                        # Legacy fallback for adapters without FIFO helpers
    ...
    async def _run_background_task(
        ...
        try:
            ...
            if turn_route.get("blocked"):
                return {
                    "final_response": turn_route.get("block_message") or "🚫 Ação bloqueada pela política do canal.",
                    "messages": [],
                    "api_calls": 0,
                    "tools": [],
                }
            if turn_route.get("label"):
                logger.info("Background %s", turn_route.get("label"))
    ...
    async def _run_agent(
        ...
        def run_sync():
            ...
            if turn_route.get("label"):
                logger.info("%s", turn_route.get("label"))
