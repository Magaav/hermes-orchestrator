...
The candidate `inspect_windows_cdp_runtime` hot operation is a read-only
fallback for pages whose ordinary DOM semantics cannot establish application
ownership or state. It correlates one optional semantic locator with bounded
DOM ancestry, JavaScript prototype/property descriptors, page revision, and a
short-lived handle. It reads descriptors without invoking getters, caps every
surface, and does not authorize arbitrary evaluation or mutation. Source tests
prove the contract only until a downloaded installed-client receipt passes.
